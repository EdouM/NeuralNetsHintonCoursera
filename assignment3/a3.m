...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % Forward Propagation 
  Z2 = model.input_to_hid * data.inputs; 
  A2 = logistic(Z2); 
  Z3 = model.hid_to_class * A2;

  class_normalizer = log_sum_exp_over_rows(Z3); 
  log_class_prob = Z3 - repmat(class_normalizer, [size(Z3, 1), 1]); 
  A3 = exp(log_class_prob);

  % Backward Propagation
  D3 = A3 - data.targets; 
  SG2 = sigmoidGradient(Z2);
  D2 = (model.hid_to_class' * D3) .* SG2;

  % Gradients
  ret.input_to_hid = (D2 * data.inputs') / size(data.inputs, 2) + model.input_to_hid * wd_coefficient; 
  ret.hid_to_class = (D3 * A2') / size(data.inputs, 2) + model.hid_to_class * wd_coefficient;
